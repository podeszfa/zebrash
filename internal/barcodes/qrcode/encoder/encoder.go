...
func Encoder_encode(content string, ecLevel ErrorCorrectionLevel, opts Options) (*QRCode, error) {
	...
	if opts.CharacterSetName != "" {
		if eci, ok := GetCharacterSetECIByName(fmt.Sprintf("%v", opts.CharacterSetName)); ok {
			encoding = eci.GetCharset()
		} else {
			return nil, fmt.Errorf("WriterException")
		}
	}
	...
	headerBits := utils.NewEmptyBitArray()

	// Append ECI segment if applicable
	if mode == Mode_BYTE && opts.CharacterSetName != "" {
		eci, ok := GetCharacterSetECI(encoding)
		if ok && eci != nil {
	...
	// main payload yet.
	dataBits := utils.NewEmptyBitArray()
	...
	if opts.VersionNumber > 0 {
		var e error
		version, e = Version_GetVersionForNumber(opts.VersionNumber)
		if e != nil {
			return nil, e
		}
		bitsNeeded := calculateBitsNeeded(mode, headerBits, dataBits, version)
		if !willFit(bitsNeeded, version, ecLevel) {
			return nil, fmt.Errorf("data too big for requested version")
		}
	} else {
	...
	headerAndDataBits := utils.NewEmptyBitArray()
...
func recommendVersion(ecLevel ErrorCorrectionLevel, mode *Mode, headerBits, dataBits *utils.BitArray) (*Version, error) {
	// Hard part: need to know version to know how many bits length takes. But need to know how many
	// bits it takes to know version. First we take a guess at version by assuming version will be
	// the minimum, 1:
	version1, _ := Version_GetVersionForNumber(1)
...
func calculateBitsNeeded(mode *Mode, headerBits *utils.BitArray, dataBits *utils.BitArray, version *Version) int {
...
func chooseMode(content string, encoding textencoding.Encoding) *Mode {
	if StringUtils_SHIFT_JIS_CHARSET == encoding && isOnlyDoubleByteKanji(content) {
...
func isOnlyDoubleByteKanji(content string) bool {
	enc := StringUtils_SHIFT_JIS_CHARSET.NewEncoder()
...
func chooseMaskPattern(bits *utils.BitArray, ecLevel ErrorCorrectionLevel, version *Version, matrix *ByteMatrix) (int, error) {
...
func chooseVersion(numInputBits int, ecLevel ErrorCorrectionLevel) (*Version, error) {
	for versionNum := 1; versionNum <= 40; versionNum++ {
		version, _ := Version_GetVersionForNumber(versionNum)
...
func willFit(numInputBits int, version *Version, ecLevel ErrorCorrectionLevel) bool {
...
func terminateBits(numDataBytes int, bits *utils.BitArray) error {
	...
	if bits.GetSize() != capacity {
		return fmt.Errorf("bits.GetSize()=%d, capacity=%d", bits.GetSize(), capacity)
	}
	return nil
}
...
func getNumDataBytesAndNumECBytesForBlockID(numTotalBytes, numDataBytes, numRSBlocks, blockID int) (int, int, error) {
	if blockID >= numRSBlocks {
		return 0, 0, fmt.Errorf("block ID too large")
	}
	...
	if numEcBytesInGroup1 != numEcBytesInGroup2 {
		return 0, 0, fmt.Errorf("EC bytes mismatch")
	}
	// 5 = 4 + 1.
	if numRSBlocks != numRsBlocksInGroup1+numRsBlocksInGroup2 {
		return 0, 0, fmt.Errorf("RS blocks mismatch")
	}
	// 196 = (13 + 26) * 4 + (14 + 26) * 1
	if numTotalBytes !=
		((numDataBytesInGroup1+numEcBytesInGroup1)*numRsBlocksInGroup1)+
			((numDataBytesInGroup2+numEcBytesInGroup2)*numRsBlocksInGroup2) {
		return 0, 0, fmt.Errorf("total bytes mismatch")
	}
...
func interleaveWithECBytes(bits *utils.BitArray, numTotalBytes, numDataBytes, numRSBlocks int) (*utils.BitArray, error) {

	// "bits" must have "getNumDataBytes" bytes of data.
	if bits.GetSizeInBytes() != numDataBytes {
		return nil, fmt.Errorf("number of bits and data bytes does not match")
	}
	...
	if numDataBytes != dataBytesOffset {
		return nil, fmt.Errorf("data bytes does not match offset")
	}

	result := utils.NewEmptyBitArray()
	...
	if numTotalBytes != result.GetSizeInBytes() { // Should be same.
		return nil, fmt.Errorf("interleaving error: %v  and %v differ", numTotalBytes, result.GetSizeInBytes())
	}

	return result, nil
}

var rsEncoder = utils.NewReedSolomonEncoder(utils.NewGaloisField(0x011D, 256, 0)) // x^8 + x^4 + x^3 + x^2 + 1

func generateECBytes(dataBytes []byte, numEcBytesInBlock int) ([]byte, error) {
	numDataBytes := len(dataBytes)
	toEncode := make([]int, numDataBytes)
	for i := range numDataBytes {
		toEncode[i] = int(dataBytes[i]) & 0xFF
	}

	eccInts := rsEncoder.Encode(toEncode, numEcBytesInBlock)

	ecBytes := make([]byte, len(eccInts))
	for i := range len(eccInts) {
		ecBytes[i] = byte(eccInts[i])
	}
...
func appendModeInfo(mode *Mode, bits *utils.BitArray) {
	_ = bits.AppendBits(mode.GetBits(), 4)
}

// appendLengthInfo Append length info. On success, store the result in "bits".
func appendLengthInfo(numLetters int, version *Version, mode *Mode, bits *utils.BitArray) error {
...
func appendBytes(content string, mode *Mode, bits *utils.BitArray, encoding textencoding.Encoding) error {
	switch mode {
	case Mode_NUMERIC:
...
func appendNumericBytes(content string, bits *utils.BitArray) {
	length := len(content)
	i := 0
	for i < length {
...
func appendAlphanumericBytes(content string, bits *utils.BitArray) error {
	length := len(content)
	i := 0
	for i < length {
		code1 := getAlphanumericCode(content[i])
		if code1 == -1 {
			return fmt.Errorf("appendAlphanumericBytes")
		}
		if i+1 < length {
...
func append8BitBytes(content string, bits *utils.BitArray, encoding textencoding.Encoding) error {
...
func appendKanjiBytes(content string, bits *utils.BitArray) error {
	enc := StringUtils_SHIFT_JIS_CHARSET.NewEncoder()
	bytes, e := enc.Bytes([]byte(content))
	if e != nil {
		return e
	...
	if len(bytes)%2 != 0 {
		return fmt.Errorf("kanji byte size not even")
	}
...
func appendECI(eci *CharacterSetECI, bits *utils.BitArray) {
	_ = bits.AppendBits(Mode_ECI.GetBits(), 4)
