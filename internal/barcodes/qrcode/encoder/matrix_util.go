...
func embedBasicPatterns(version *Version, matrix *ByteMatrix) error {
...
func embedTypeInfo(ecLevel ErrorCorrectionLevel, maskPattern int, matrix *ByteMatrix) error {
	typeInfoBits := utils.NewEmptyBitArray()
...
func maybeEmbedVersionInfo(version *Version, matrix *ByteMatrix) error {
	if version.GetVersionNumber() < 7 { // Version info is necessary if version >= 7.
		return nil // Don't need version info.
	}
	versionInfoBits := utils.NewEmptyBitArray()
...
func embedDataBits(dataBits *utils.BitArray, maskPattern int, matrix *ByteMatrix) error {
	bitIndex := 0
	direction := -1
	...
	if bitIndex != dataBits.GetSize() {
		return fmt.Errorf("not all bits consumed: %v/%v", bitIndex, dataBits.GetSize())
	}
	return nil
}
...
func makeTypeInfoBits(ecLevel ErrorCorrectionLevel, maskPattern int, bits *utils.BitArray) error {
	if !QRCode_IsValidMaskPattern(maskPattern) {
		return fmt.Errorf("invalid mask pattern")
	}
	...
	maskBits := utils.NewEmptyBitArray()
...
func makeVersionInfoBits(version *Version, bits *utils.BitArray) error {
	bits.AppendBits(version.GetVersionNumber(), 6)
	bchCode, _ := calculateBCHCode(version.GetVersionNumber(), matrixUtil_VERSION_INFO_POLY)
	bits.AppendBits(bchCode, 12)

	if bits.GetSize() != 18 { // Just in case.
		return fmt.Errorf("should not happen but we got: %v", bits.GetSize())
	}
...
func embedDarkDotAtLeftBottomCorner(matrix *ByteMatrix) error {
	if matrix.Get(8, matrix.GetHeight()-8) == 0 {
		return fmt.Errorf("embedDarkDotAtLeftBottomCorner")
	}
...
func maybeEmbedPositionAdjustmentPatterns(version *Version, matrix *ByteMatrix) {
