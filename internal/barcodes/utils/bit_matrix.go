...
func NewBitMatrix(width, height int) (*BitMatrix, error) {
	if width < 1 || height < 1 {
		return nil, fmt.Errorf("both dimensions must be greater than 0")
	}
	rowSize := (width + 31) / 32
...
func ParseStringToBitMatrix(stringRepresentation, setString, unsetString string) (*BitMatrix, error) {
	if stringRepresentation == "" {
		return nil, fmt.Errorf("empty string representation")
	}
		...
		if c := stringRepresentation[pos]; c == '\n' || c == '\r' {
			if bitsPos > rowStartPos {
				if rowLength == -1 {
					...
					return nil, fmt.Errorf("row length do not match")
				}
				rowStartPos = bitsPos
				nRows++
			}
...
func (b *BitMatrix) Bounds() image.Rectangle {
	return image.Rect(0, 0, b.GetWidth(), b.GetHeight())
}
...
func (b *BitMatrix) Xor(mask *BitMatrix) error {
	if b.width != mask.width || b.height != mask.height || b.rowSize != mask.rowSize {
		return fmt.Errorf("input matrix dimensions do not match")
	}
...
func (b *BitMatrix) SetRegion(left, top, width, height int) error {
	if top < 0 || left < 0 {
		return fmt.Errorf("left and top must be nonnegative")
	}
	if height < 1 || width < 1 {
		return fmt.Errorf("height and width must be at least 1")
	}
	right := left + width
	bottom := top + height
	if bottom > b.height || right > b.width {
		return fmt.Errorf("the region must fit inside the matrix")
	}
